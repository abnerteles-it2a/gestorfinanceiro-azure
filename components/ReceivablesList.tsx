...
export const ReceivablesList: React.FC<{ readOnly?: boolean; hideAddForm?: boolean; reloadKey?: number }> = ({ readOnly=false, hideAddForm=false, reloadKey }) => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/query', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ type: 'receivables_list', data: { month: monthFilter, status: pStatus } }) });
      const j = await r.json();
      setItems((j.rows || []));
    } catch {
        setErrorToast('Não foi possível sincronizar os dados de recebimento. Verifique a internet e tente novamente.');
    }
    setLoading(false);
  };
  ...
  const onAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTitle(''); setAmount(''); setIssueDate(''); setCategory(''); setCostCenterId(''); setCustomer(''); setNotes(''); setInstallments(1);
    } catch {}
  };

  const onDelete = async (id: string) => {
    try { await fetch('/api/query', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ type: 'receivables_delete', data: { id } }) }); setItems(prev=>prev.filter(i=>i.id!==id)); } catch {}
    setDeleteConfirmId('');
  };
  ...
  const onMarkReceived = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMarkingId(''); setReceivedAmount(''); setReceivedDate(new Date().toISOString().slice(0,10)); setAccountId(''); setPaymentMethod(''); setDescription('');
    } catch {}
  };
  ...
  const submitEdit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const r = await fetch('/api/query', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ type: 'receivables_update', data: { id: editId, title: editTitle, amount: parseCurrencyInput(editAmount), issueDate: (editIssueDate || null), dueDate: (editDueDate || null), category: (editCategory || null), costCenterId: (editCostCenterId || null), customer: (editCustomer || null), notes: (editNotes || null), status: editStatus } }) });
      const j = await r.json();
      const updated = (j.rows || [])[0];
      setItems(prev => prev.map(i => i.id === updated.id ? updated : i));
      cancelEdit();
      ...
      render: (it) => (
        <div className="flex items-center justify-end gap-1.5">
          {deleteConfirmId === it.id ? (
            ...
            <>
              <button onClick={(e)=>{ e.stopPropagation(); setMarkingId(it.id); }} title="Receber" className="p-1 rounded-full text-green-600 hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors">
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col">
        ...
        filters={
          <>
            <div className="flex bg-slate-100/50 dark:bg-slate-800/50 p-1.5 rounded-lg border border-slate-200/60 dark:border-slate-700/60">
              ...
              <button type="button" onClick={() => setStatusFilter('received')} className={`px-4 py-1 text-[11px] font-bold tracking-widest uppercase rounded-md transition-all ${statusFilter === 'received' ? 'bg-white dark:bg-slate-700 text-emerald-600 dark:text-emerald-400 shadow-sm ring-1 ring-slate-200/50 dark:ring-slate-600' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Recebidas</button>
      ...
      <div className="p-6 relative">
        ...
        <form onSubmit={onAdd} className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-6 bg-slate-50/50 dark:bg-slate-900/10 p-5 rounded-2xl border border-slate-200 dark:border-slate-800/60 shadow-sm relative overflow-hidden">
          ...
          <FormField label="Valor" className="md:col-span-2">
            <Input value={amount} onChange={e=>setAmount(e.target.value)} type="number" step="0.01" placeholder="Ex: 150.00" required />
          </FormField>
          ...
          <FormField label="Categoria" className="md:col-span-2">
            <Select value={category} onChange={e=>setCategory(e.target.value)}>
              ...
              {categories.filter(c=>c.type==='Entrada').map(c=> (<option key={c.id} value={c.name}>{c.name}</option>))}
            </Select>
          </FormField>
      ...
      <DataTable 
        ...
        pagination={true}
        rowsPerPage={5}
        rowClassName={(it) => it.status === 'received'
          ? 'bg-slate-50/20 dark:bg-slate-800/10 hover:bg-slate-100/40 dark:hover:bg-slate-800/20'
          : 'bg-emerald-50/20 dark:bg-emerald-950/5 hover:bg-emerald-100/35 dark:hover:bg-emerald-950/15 border-l-2 border-l-emerald-300 dark:border-l-emerald-800'
        }
      />
      ...
      {editId && !readOnly && (
        <form onSubmit={submitEdit} className="mt-4 grid grid-cols-1 md:grid-cols-12 gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/60 p-5 rounded-2xl shadow-sm ring-1 ring-slate-200/50 dark:ring-transparent relative overflow-hidden">
          ...
          <div className="md:col-span-12 border-b border-slate-100 dark:border-slate-800/60 pb-2 mb-2">
            <h4 className="text-[12px] font-bold text-slate-500 uppercase tracking-widest">Editando Recebimento</h4>
          </div>
          ...
          <FormField label="Cliente" className="md:col-span-3">
            <Input value={editCustomer} onChange={e=>setEditCustomer(e.target.value)} placeholder="Cliente" />
          </FormField>
      ...
      {markingId && !readOnly && (
        <form onSubmit={onMarkReceived} className="mt-4 grid grid-cols-1 md:grid-cols-12 gap-3 bg-white dark:bg-slate-900 border border-emerald-200 dark:border-emerald-800/60 p-5 rounded-2xl shadow-sm ring-1 ring-emerald-200/50 dark:ring-transparent relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500 rounded-l-2xl"></div>
          
          <div className="md:col-span-12 border-b border-emerald-100 dark:border-emerald-800/40 pb-2 mb-2">
            <h4 className="text-[12px] font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-widest">Confirmação de Recebimento</h4>
          </div>

          <FormField label="Conta Alvo" className="md:col-span-3">
          ...
          <FormField label="Valor Efetivamente Recebido" className="md:col-span-3">
            <Input value={receivedAmount} onChange={e=>setReceivedAmount(e.target.value)} type="number" step="0.01" placeholder="Ex: 50.00" />
          </FormField>
          
          <FormField label="Data do Recebimento" className="md:col-span-3">
            <Input value={receivedDate} onChange={e=>setReceivedDate(e.target.value)} type="date" />
          </FormField>
          
          <FormField label="Método de Recebimento" className="md:col-span-3">
